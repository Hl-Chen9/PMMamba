...
def complexTo2Channels(target_array):
    ...
    # output_array = get2channels(target_array)
...
def get2channels(target_array, power_order=2):
    """ get 2 channels out of complex number """
    # target_array = np.abs(target_array)
    target_array = np.expand_dims(target_array, axis=3)
    target_array = np.concatenate([target_array.real, target_array.imag], axis=3)
    target_array = pow(target_array, power_order)
    return target_array
